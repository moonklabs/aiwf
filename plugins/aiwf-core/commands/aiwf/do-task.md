---
description: Execute sprint task with guided workflow and quality gates
---
...
## 6 · 자동 검증 (Anthropic Long-Running Agent Pattern)

**🔍 태스크 완료 전 필수 검증:**

태스크 작업이 완료되면 자동 검증 훅을 실행합니다:
...
**검증 항목:**
- 유닛 테스트 (npm test / yarn test / pytest)
...
1. **VERIFICATION_PASSED** 인 경우:
   - 태스크 파일의 `passes` 섹션 업데이트:
     ```yaml
     ## Passes
     status: true
     verified_at: [현재 타임스탬프]
     verified_by: automated
     ```
   - 다음 단계(코드 리뷰)로 진행

2. **VERIFICATION_FAILED** 인 경우:
   - 실패한 검증 항목 분석
   - 적절한 서브에이전트 호출하여 문제 해결:
     - 빌드 실패 → `react-build-fixer` 또는 `backend-build-fixer`
     - 타입 오류 → `auto-error-resolver`
     - 테스트 실패 → `debug-specialist`
   - 문제 해결 후 검증 재실행
   - **중요**: 모든 검증이 통과할 때까지 완료 선언 금지
