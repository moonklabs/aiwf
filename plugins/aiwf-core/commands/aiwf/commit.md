---
description: Git commit with AIWF conventions and intelligent grouping
---
...
**🏷️ 체크포인트 태그 생성 (Anthropic Long-Running Agent Pattern)**:

태스크가 완료된 커밋인 경우 자동으로 체크포인트 태그를 생성합니다:

```bash
# 태스크 완료 시 체크포인트 생성
if [[ "$COMMIT_MESSAGE" =~ (완료|completed|done|finish) ]] || [[ "$TASK_STATUS" == "completed" ]]; then
    CHECKPOINT_TAG="aiwf-checkpoint-${TASK_ID}-$(date +%Y%m%d-%H%M)"
    git tag "$CHECKPOINT_TAG"
    echo "🏷️ 체크포인트 생성: $CHECKPOINT_TAG"
fi
```

**체크포인트 활용:**
- 세션 복구: `/aiwf:restore` 명령어로 체크포인트에서 복구 가능
- 진행 추적: `aiwf-progress.md`에 체크포인트 기록
- 롤백: 문제 발생 시 안전한 상태로 빠르게 복구

## 7 · Generate Changelog (Optional)

After successful commits:

- Ask user: "Commits completed successfully. Would you like to update the Changelog? (y/n)"
