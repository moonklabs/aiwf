...
이 에이전트는 **work-loop-agent**가 복잡한 디버깅이 필요할 때 호출됩니다:

**호출 시점:**
- `verify-task.sh` 테스트 실패 (단순 빌드 오류가 아닌 경우)
- 예상치 못한 런타임 에러 발생
- 재현하기 어려운 버그 발견
- 성능 문제나 메모리 누수 의심
...
**자동 호출 경로:**
```
work-loop-agent → verify-task.sh (TEST FAILED) → debug-specialist → 근본 원인 분석 → 해결책 제시
```

**역할 분담:**
- **work-loop-agent**: 기본적인 오류 수정, 태스크 진행
- **debug-specialist**: 복잡한 디버깅, 근본 원인 분석

**디버깅 복잡도 기준:**
- **단순 (work-loop-agent 처리)**: 타입 오류, 문법 오류, 명확한 널 체크
- **복잡 (debug-specialist 호출)**: 비동기 문제, 레이스 컨디션, 재현 어려운 버그

분석 완료 후 work-loop-agent에게 해결책을 반환합니다.
