...
커밋 실행 전 아키텍처 변경사항 확인:

```bash
# 스테이징된 파일 중 아키텍처 관련 변경 확인
ARCH_FILES=$(git diff --cached --name-only | grep -E "(package\.json|config/|docker|infrastructure/|.*\.yaml|.*\.yml|.*\.config\.)")

if [ -n "$ARCH_FILES" ]; then
  echo "🏗️ 아키텍처 관련 변경사항 감지:"
  echo "$ARCH_FILES"
  ...
  if [[ "$ARGUMENTS" == *"YOLO"* ]]; then
    echo -e "\n🤖 YOLO 모드: ADR이 자동으로 관리됩니다"
  else
    echo -e "\n💡 권장: /create_adr 또는 /update_adr 명령어 사용"
  fi
fi
```
